...
pub type InteractiveMeasurement<DI, DO, MI, MO, S, Q> =
    Measurement<DI, AllDomain<Queryable<S, Q, <DO as Domain>::Carrier>>, MI, MO>;
    ...
    (DI, MI): MetricSpace,
{
    ...
    fn update(self, d_out_query: &MO::Distance) -> Fallible<Self> {
        Ok(Self {
            d_out_budget: self.d_out_budget.neg_inf_sub(d_out_query)?,
            ..self
        })
    }
    ...
    ) -> Fallible<(Self, DO::Carrier)>
    where
        MO::Distance: Clone + PartialOrd + InfSub,
    {
...
type AcQueryable<DI, DO, MI, MO> =
    Queryable<AcState<DI, DO, MI, MO>, AcQuery<DI, DO, MI, MO>, <DO as Domain>::Carrier>;
type AcMeasurement<DI, DO, MI, MO> =
    InteractiveMeasurement<DI, DO, MI, MO, AcState<DI, DO, MI, MO>, AcQuery<DI, DO, MI, MO>>;
    ...
    (DI, MI): MetricSpace
{
    AcMeasurement::new(
        ...
        Function::new(enclose!(
            ...
            move |arg: &DI::Carrier| -> AcQueryable<DI, DO, MI, MO> {
                AcQueryable::new(
        ...
        PrivacyMap::new_fallible(move |d_in| {
            if d_in <= &d_in_budget {
                return fallible!(FailedRelation);
            }
...
mod tests {
    ...
    fn make_dummy_meas<TO: From<i32> + CheckNull>(
    ) -> Measurement<AllDomain<i32>, AllDomain<TO>, AbsoluteDistance<f64>, MaxDivergence<f64>> {
        Measurement::new(
            AllDomain::new(),
            AllDomain::new(),
    ...
    fn test_adaptive_comp_simple() -> Fallible<()> {
        ...
        let adaptive = make_adaptive_composition(
            meas1.input_domain.clone(),
            meas1.output_domain.clone(),
            meas1.input_metric.clone(),
            meas1.output_measure.clone(),
            d_in_budget,
            d_out_budget,
        );
    ...
    fn test_adaptive_composition_chain() -> Fallible<()> {
        ...
        let data = vec![
            0.6, 2.8, 6.0, 9.4, 8.9, 7.7, 5.9, 3.4, 8.0, 2.4, 4.4, 7.1, 6.0, 3.2, 7.1,
        ];
        let mut queryable = adaptive.invoke(&data)?;
        ...
        // Noisy count
        let measurement1 = (make_count()?
            >> make_base_discrete_laplace_linear(1.0 / d_out_query, Some(count_bounds))?)?
        .into_poly();
        ...
        // Noisy sum
        let measurement2 = (make_clamp(val_bounds)?
