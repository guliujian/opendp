...
impl MetricSpace for (AnyDomain, AnyMetric) {}
...
impl<DI: 'static + Domain, DO: 'static + Domain, MI: 'static + Metric, MO: 'static + Measure>
    IntoAnyMeasurementExt for Measurement<DI, DO, MI, MO>
where
    DI::Carrier: 'static,
    DO::Carrier: 'static,
    MI::Distance: 'static,
...
impl<DO: 'static + Domain> IntoAnyMeasurementOutExt
    for Measurement<AnyDomain, DO, AnyMetric, AnyMeasure>
where
    DO::Carrier: 'static,
{
    fn into_any_out(self) -> AnyMeasurement {
...
/// A Postprocessor with all generic types filled by Any types. This is the type of Postprocessors
/// passed back and forth over FFI.
pub type AnyPostprocessor = Postprocessor<AnyDomain, AnyDomain>;

/// A trait for turning a Postprocessor into an AnyPostprocessor. We can't used From because it'd conflict
/// with blanket implementation, and we need an extension trait to add methods to Measurement.
pub trait IntoAnyPostprocessorExt {
    fn into_any(self) -> AnyPostprocessor;
}

impl<DI: 'static + Domain, DO: 'static + Domain>
    IntoAnyPostprocessorExt for Postprocessor<DI, DO>
where
    DI::Carrier: 'static,
    DO::Carrier: 'static,
{
    fn into_any(self) -> AnyPostprocessor {
        AnyPostprocessor::new(
            AnyDomain::new(self.input_domain),
            AnyDomain::new(self.output_domain),
            self.function.into_any(),
        )
...
mod tests {
    ...
    fn test_any_chain() -> Fallible<()> {
        ...
        let m1 = measurements::make_base_laplace::<AllDomain<_>>(
            0.0, None,
        )?
        .into_any();
