...
impl<DQ: Domain, DA: Domain> Debug for QueryableDomain<DQ, DA> {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        write!(f, "QueryableDomain({}, {})", type_name!(DQ), type_name!(DA))
    }
}
impl<DQ: Domain, DA: Domain> QueryableDomain<DQ, DA> {
    pub fn new(query_domain: DQ, answer_domain: DA) -> Self {
        QueryableDomain {
            query_domain,
            answer_domain,
        }
...
impl<DQ: Domain, DA: Domain> Clone for QueryableDomain<DQ, DA> {
    fn clone(&self) -> Self {
        Self {
            query_domain: self.query_domain.clone(),
            answer_domain: self.answer_domain.clone(),
        }
    }
}
